...
\begin{figure}[!h]
\centering
\includegraphics[width=140mm]{images/nnser_lightcurve_automated.png}
\caption{Light curve of the target object, NN Ser, labelled '7'. The vertical axis is the raw flux measurements calculated by SExtractor.  Comment: Replace this with a lightcurve plotted from matplotlib or something. Better x-axis labels.}
\label{fig:nnserlightcurve}
\end{figure}
...
\begin{figure}[!h]
\centering
\includegraphics[width=140mm]{images/nnser_comparison_lightcurve_automated.png}
\caption{Light curve of the comparison object, labeled '0'. The vertical axis is the raw flux measurements calculated by SExtractor.  Comment: Replace this with a lightcurve plotted from matplotlib or something. Better x-axis labels.}
\label{fig:nnsercomparison}
\end{figure}


To reduce the same run using the standard pipeline, we defined apertures for the objects we wanted light curves for. In this case, we are interested in the objects labeled \emph{7} (the target 'NN Ser') and \emph{0} (a comparison star) in figure \ref{fig:nnserfield} 

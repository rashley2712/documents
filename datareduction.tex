...
For each frame in the data run (which could consist of several to several thousand frames), we extract a bitmap of the image which consist of a 2D pixel map with the CCD counts (or ADU) for each pixel and pass this to SExtractor for source extraction. 
...
\end{array}  
$
 This is an optional step and can be configured before running the automated pipeline. The default 
